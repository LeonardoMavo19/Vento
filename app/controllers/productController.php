<?php
	...
	class productController extends mainModel{
		...
		public function registrarProductoControlador(){
			...
			$precio_compra_raw=$this->limpiarCadena($_POST['producto_precio_compra']);
			$precio_venta_raw=$this->limpiarCadena($_POST['producto_precio_venta']);

			// Normalizar separador decimal y convertir a float (guardamos en USD)
			$precio_compra_raw = str_replace(',', '.', $precio_compra_raw);
			$precio_venta_raw = str_replace(',', '.', $precio_venta_raw);

			$precio_compra = floatval($precio_compra_raw);
			$precio_venta = floatval($precio_venta_raw);
